# Примеры работы с сервисом Distributed Train от Cloud.ru по ssh
...
Первое что необходимо сделать - подключиться к запущенному инстансу Jupyter Server (free или gpu) по ssh. Процесс подключения подробно описан в документации сервиса и в интерфейсе Distributed Train.
...
#### 2. Посредством отправки задачи обучения на кластер из free Jupyter Server.
   ...
   3. Далее следить за процессом обучения можно как из интерфейса Distributed Train, так и выполнив в консоли следующую команду: `python3 start_job.py jobs logs lm-mpi-job-b604a982-6158-4f3d-b11c-efa1229ddb34`